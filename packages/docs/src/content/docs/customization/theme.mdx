...
Filter Sphere includes an `ErrorBoundary` component that wraps `FilterGroupContainer` and `SingleFilterContainer` to catch and handle rendering errors gracefully. This prevents the entire filter UI from crashing when an individual rule or group encounters an error.

You can try out the error boundary by using the example below.

import { ErrorBoundaryExample } from "~/components/examples/error-boundary-example.tsx";

<div className="not-content">
  <ErrorBoundaryExample client:load />
</div>
...
<Tabs>
  <TabItem label="components" icon="seti:svg">
    <ThemeAnatomy target="components" client:load />
      ...
      You can override the components to customize the appearance of the Filter Sphere. The components include: `Button`, `Input`, `Select`, `MultipleSelect` and `ErrorBoundary`.
  ...
  <TabItem label="templates" icon="seti:plan">
    ...
    See [Schema Metadata](/fn-sphere/customization/schema-metadata/) for an example that reads application-owned field metadata from a custom `FieldSelect`.
