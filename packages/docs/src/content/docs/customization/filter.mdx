...
`.nullish()`. They keep the filters and input UI for their underlying string,
number, date, boolean, enum, literal-union, or array type, and also offer
...
const data = [
  { title: "Compatibility guide", score: 92, publishedAt: new Date() },
  { title: undefined, score: null, publishedAt: null },
  { title: "Filter presets", score: 64, publishedAt: undefined },
];
...
Custom filters remain explicit. A filter whose first input is `z.string()` is
not offered for a `z.string().optional()` field. Declare the accepted nullish
values and handle them in the implementation:
