...
Field descriptions are Zod schema metadata. See [Using Schema Metadata](/fn-sphere/guides/best-practices/#using-schema-metadata) to learn how to define typed metadata keys and read them in custom consumers.
...
const customMapFieldName = (field: FilterField) => {
  const fieldPath = field.path.length ? field.path.join(".") : "root";
  return z.globalRegistry.get(field.fieldSchema)?.description ?? fieldPath;
};
...
export const defaultMapFieldName: (field: FilterField) => string = (field) => {
  ...
  const description = z.globalRegistry.get(field.fieldSchema)?.description;
  if (description) {
    return description;
  }
