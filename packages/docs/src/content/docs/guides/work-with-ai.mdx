...
```
Create a custom filter function that checks if a string matches a regex pattern
```

```
Set up multi-field sorting for my user list with name ascending and age descending
```

```
Add localization support to my filter builder with English and Chinese
```
